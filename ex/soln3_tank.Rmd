...
rbind(quantile = quant_interval, hdi = interval)

```

The HDI is quite a bit narrower than the quantile interval.
There is a more complete function for doing this in the git repository in: `vu_advstats_students/r/mh.r`
:::
...
//	s ~ uniform(1, floor(n_latent));
	for(i in 1:nobs) {
		if(floor(N_latent) < s[i]) {
			...
			target += -log(floor(N_latent)); // discrete uniform distribution with a min of 1 and max of N_latent
		}
	}
...
samps_sorted = sort(tank_samps_d[,'N'])
...
The results very similar, but now the mode is correctly and exactly included!
