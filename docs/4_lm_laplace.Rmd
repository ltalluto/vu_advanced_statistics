...
apr = seq(-2, 2, length.out = 100)
bpr = seq(-2, 2, length.out = 100)
spr = seq(0, 4, length.out = 100)
a = mod_map$par[1]
b = mod_map$par[2]
s = mod_map$par[3]

par(mfrow=c(2, 3), bty='l', mar=c(4,4,0,0))
plot(apr, exp(sapply(apr, function(aa) log_liklihood(aa, b, s, data$x, data$y))), type='l', xlab='', ylab='pr(a | x,b,s)', col='red', lwd=2, xaxt='n')
plot(bpr, exp(sapply(bpr, function(bb) log_liklihood(a, bb, s, data$x, data$y))), type='l', xlab='', ylab='pr(b | x,a,s)', col='red', lwd=2, xaxt='n')
plot(spr, exp(sapply(spr, function(ss) log_liklihood(a, b, ss, data$x, data$y))), type='l', xlab='', ylab='pr(s | x,a,b)', col='red', lwd=2, xaxt='n')
plot(apr, exp(sapply(apr, function(aa) log_prior(aa, b, s))), type='l', xlab='a', ylab='pr(a)', col='blue', lwd=2)
plot(bpr, exp(sapply(bpr, function(bb) log_prior(a, bb, s))), type='l', xlab='b', ylab='pr(b)', col='blue', lwd=2)
plot(spr, exp(sapply(spr, function(ss) log_prior(a, b, ss))), type='l', xlab='s', ylab='pr(s)', col='blue', lwd=2)
```
