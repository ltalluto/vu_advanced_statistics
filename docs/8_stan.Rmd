...
* Estimates models using Hamiltonian Monte Carlo
* Programs are written in a .stan file
* The Stan interpreter (which can be called from R) translates Stan code to C++ code
* This program is executed from within R, posterior samples returned to R
...
```{r eval=FALSE}
library(rstan)
fit = rstan(stancode, iter=5000, data = data)
...
<div class="left lt">
* Stan variables are less flexible than in R
* Stan is an example of a *stongly-typed* language
* Variables must be declared, types are fixed
* Container dimensions must be specified ahead of time
...
* Here, the model computes $pr(y | \mu, \sigma)$ using the **normal PDF**
...
## 4. Inference: Conditional Response Curves
