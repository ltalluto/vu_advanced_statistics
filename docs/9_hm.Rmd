...
pl_int_pool = mcmc_intervals(samples_pool, pars="p") + xlim(c(0,1))
pl_int_pool
```
</div>
...
pl_int_nopool = mcmc_intervals(samples_nopool) + xlim(0,1)
pl_int_nopool
```
</div>
...
<div class="right rt">
```{stan output.var="frog1_ppool", cache = TRUE, warning=FALSE}
data {
	int <lower=0> n; // number of data points
	int <lower=0> surv [n]; // number surviving
	int <lower=0> tadpole [n]; // number of tadpoles in the pond
...
fit_ppool = sampling(frog1_ppool, data = standat, iter=2000, refresh=0, chains=1)
samples_ppool = as.matrix(fit_ppool, pars="p")
pl_int_ppool = mcmc_intervals(samples_ppool) + xlim(0,1)
```
</div>

## Partial pooling is a (small) compromise

<div class="left lt">

```{r echo = FALSE, fig.height = 12}
grid.arrange(pl_int_pool, pl_int_nopool, pl_int_ppool, nrow=3, ncol=1, heights = c(0.5, 1, 1))
```
</div>
...
hparams = as.matrix(fit_ppool, pars=c("mua", "siga"))
hparams = cbind(hparams, mup = plogis(hparams[,"mua"]))
mcmc_intervals(hparams, pars="mup") + xlim(c(0,1))
```
</div>
