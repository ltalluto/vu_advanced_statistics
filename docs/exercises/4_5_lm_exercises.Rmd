...
For these exercises, you will need the `Howell1` dataset from the `rethinking` package.

```{r message=FALSE, results="hide"}
## if you don't have the rethinking package, run these lines
...
2. Design a regression model with `height` as the **response** (i.e., outcome, y) variable. For your initial model, use only `weight` as a predictor.
    a. Write out the model, using the syntax from [slide 6 on Tuesday](../4_lm_laplace.html#(6)). Be sure that all unknowns (including parameters) appear on the left side of either ~ or =.
...
6. Look at [slide 10 from Thursday](../5_posterior_inference.html#(10)). Many people have trouble understanding why we add a column of 1s to the prediction dataset, and the `%*%` operator, and also the `apply` statement. Try the help file for both of them (use ?"%*%" to get help for the operator). Also run the code yourself and inspect the output. 
