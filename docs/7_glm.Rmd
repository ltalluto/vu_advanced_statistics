...
<div class="left lt">
* Our linear model last week used a single x-variable: $\mathbb{E}(y) = a + bx$
...
pl = ggplot(Howell1, aes(x=weight, y = height, colour=age_cat)) + geom_point() + theme_minimal()
```
</div>
...
log_liklihood1 = function(a, b_child, b_young, b_weight, s, height, weight, agecat_child, agecat_young) {
  mu = a + b_child*agecat_child + b_young*agecat_young + b_weight * weight
  sum(dnorm(height, mu, s, log=TRUE))
}

## equivalent of lm(height ~ age_cat * weight)
log_liklihood2 = function(a, b_child, b_young, b_weight, b_cw, b_yw, s, 
                          height, weight, agecat_child, agecat_young) {
  mu = a + b_child*agecat_child + b_young*agecat_young + b_weight * weight  + 
    b_cw * agecat_child * weight + b_yw * agecat_young * weight
  sum(dnorm(height, mu, s, log=TRUE))
}
...
```{r}
fit = optim(c(a=0.5, b=0), log_posterior, data = tsuga[n > 0], method = "Nelder-Mead", control=list(fnscale=-1))
...
* Linear regression is a special case of the GLM, where
   - $\mathcal{D}$ is a normal distribution with mean $\mathbb{E}(y)$ and constant variance $\sigma^2 = /phi$
...
* For counts where the number of trials is unkown or non-sensical
